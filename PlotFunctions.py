...
def all_plots(record, bang_data, record_each, n_iter, t_T, t_R, dt, xT, v):
	...
	ax.step(np.linspace(0, (t_T+t_R)*1000, p), bang_data.signal[0], lw=2, where = 'post', label="Sym. bangbang")
	...
	ax.plot(np.linspace(0, (t_T+t_R)*1000, p), bang_data.position[0], lw=2, label="Sym. bangbang")
	...
	ax.plot(np.linspace(0, (t_T+t_R)*1000, p), bang_data.velocity[0], lw=2, label="Sym. bangbang")
